...
namespace UB._1
{
    public partial class SubjectWindow : Form
    {
        ...
        {
            string CommandText = "SELECT * FROM Предмет";
            cn.Open();
        ...
        private void addSubject(object sender, EventArgs e)
        {
            ...
            AddSubject(Convert.ToUInt32(textBox1.Text), textBox2.Text, textBox3.Text, Convert.ToUInt32(textBox4.Text), Convert.ToUInt32(textBox5.Text));
            PrintSubject();
        }

        private void AddSubject(uint id, string name, string shortName, uint count, uint idDept)
        {
            cn.Open();
            try
            {
